    ...
    try {
        ...
        if (!checkRoomId.membersOnly)
            return res.status(400).json({
                message: '멤버 온니 스터디방이 아닙니다. 멤버 온니 스터디 방만 신청할 수 없습니다.',
            });

        if (checkRoomId.id === applicantId)
            return res
                .status(400)
                .json({ message: '자신이 만든 방에 참가 신청 요청을 할 수 없습니다.' });
...
applicantsRouter.put('/apply/check', login_required, async function (req, res, next) {
    try {
        ...
        if (!applicantId || !roomId)
            return res.status.json({ message: 'appicantIds나 roomId가 넘어오지 않았습니다.' });
        ...
        if (checkRoom.id !== id)
            return res
                .status(400)
                .json({ message: '이 방의 방장이 아닙니다. 신청자 승인 권한이 없습니다.' });
        ...
        await Promise.all([
            applicantsService.delete({ applicantId, roomId }),
            userStudyRoomsService.addMember({ roomId, updateChange }),
        ]).then((result) => (addmember = result[1].members));

        return res.status(200).json({
            message: '성공적으로 스터디룸 멤버에 추가되었습니다.',
            roomMembers: addmember,
        });
    } catch (error) {
        next(error);
    }
...
applicantsRouter.get('/applicants/:roomId', login_required, async function (req, res, next) {
    try {
        ...
        if (!roomId)
            return res.status(400).json({ message: 'roomId 를 파라미터를 통해 보내주세요.' });

        const applicantsList = await applicantsService.getLists({ roomId });
        if (!applicantsList)
            return res.status(500).json({ message: '리스트를 불러오지 못했습니다.' });
        ...
        const applicantsListWithName = await Promise.all(
            applicantsList.map(async (applicant) => {
                ...
                const userName = await userAuthService
                    .getUserInfo({ user_id })
                    .then((info) => info.name);
                const userURL = await userAuthService
                    .getUserInfo({ user_id })
                    .then((info) => info.profileUrl);
                return { userName, ...applicant.toObject(), userURL };
            }),
        );
    ...
    async function (req, res, next) {
        try {
            ...
            if (!deletedApplication)
                return res.status(400).json({ message: '삭제가 정상적으로 되지 않았습니다.' });

            return res.status(200).json({
                message: '성공적으로 신청자 리스트에서 삭제되었습니다.',
                deletedApplication,
            });
        } catch (error) {
            next();
...
applicantsRouter.delete(
    ...
    async function (req, res, next) {
        try {
            ...
            const delMember = await userStudyRoomsService
                .delMember({ roomId, updateChange })
                .then((result) => result.members);
            if (!delMember)
                return res.status(500).json({ message: ' 해당 멤버를 퇴장시키지 못했습니다.' });

            return res.status(200).json({
                message: '성공적으로 스터디룸 멤버에서 제거되었습니다.',
                roomMembers: delMember,
            });
        } catch (error) {
            next(error);
        }
