const ChatMainText = ({ chattingBoxRef, chatAll, userDatas, user }) => {
  return (
    <div ref={chattingBoxRef} className="relative w-full p-6 overflow-y-auto h-[72%]">
            ...
            <>
              {name[0] === `${user?.name}` ? (
                ...
                <li className="flex justify-end">
                  ...
                  <img className="rounded-full bg-cover w-10 h-10 ml-2" src={user?.profileUrl} />
                </li>
              ) : (
                // 상대방 메시지
                <li className="flex justify-start">
                  {/* <div className="grid mr-2"> */}
                  <img className="rounded-full bg-cover w-10 h-10 mr-2" src={userI?.profileUrl} alt="" />
